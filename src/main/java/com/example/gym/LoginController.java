...
public class LoginController {
        ...
        try {
            ...
            if(si_username.getText().isEmpty() || si_password.getText().isEmpty())
            {
                ...
                {   data.setUsername(si_username.getText());
                    if(si_password.getText().equals("hamza1212"))
                        data.setIamAdmin(true);
                    else
                        data.setIamAdmin(false);
                    si_username.setText("");
                    si_password.setText("");
    ...
    @FXML
    private TextField su_add;
    public void signup()
    {
        String sql="INSERT INTO admin (username,password,email) values (?,?,?)";
        connect = Database.connectDb();
        try {
            if(su_username.getText().isEmpty() || su_password.getText().isEmpty() ||su_password.getText().length()<3)
                alertError();
            else
            {   if( ! su_add.getText().equals("hamza1212"))
            {Alert alert =new Alert(Alert.AlertType.ERROR);
                alert.setContentText("Donner le code de root ");
                alert.showAndWait();
            return;}
                prepare=connect.prepareStatement(sql);
